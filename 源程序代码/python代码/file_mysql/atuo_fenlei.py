...
def write_sql(id,classs):
    db = pymysql.Connection(host="localhost", port=33060, user="root", password="582607", database="wordread",
                            charset="utf8")
    cursor = db.cursor()
    sql = "update word_info set type='" + classs + "'  where word_info.index=" + str(id)
    try:
        cursor.execute(sql)
        print(sql)
        db.commit()
...
# 链接mysql数据库
conn = pymysql.Connection(host="localhost", port=33060, user="root", password="582607", database="wordread",
                            charset="utf8")
cursor = conn.cursor()
cursor=conn.cursor()
...
sql = "SELECT word_info.index,title1,title2,content FROM word_info "
# print sql
cursor.execute(sql)
